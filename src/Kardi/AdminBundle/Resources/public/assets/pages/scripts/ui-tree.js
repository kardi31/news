    ...
    var ajaxTreeSample = function () {
        ...
        $("#tree_4").jstree({
            "core": {
                ...
                'data': {
                    'url': function (node) {
                        return Routing.generate('kardi_admin_menu_show_menu_data');
                    }
                }
            },
            "types": {
            ...
            $.ajax({
                url: Routing.generate('kardi_admin_menu_move_menu_item'),
                type: 'POST',
                data: {'newTree': data.new_instance.get_json('#')},
                dataType: 'json',
                done: function (data) {
            ...
            .on('ready.jstree', function (e, data) {
                $('li[role="treeitem"]').each(function () {
                    var id = $(this).attr('id');
                    var route = Routing.generate('kardi_admin_menu_delete_item', {
                            'id': id
                        })
                    ;
                    var deleteButton = '<a href="' + route + '"><i class="fa fa-times"></i></a>';
                    $(deleteButton).insertAfter($('#'+id+'_anchor'));
                });
            });
        ;
